...
def pip_audit(session: Session) -> None:
    ...
    session.run("pip-audit", "--requirement", str(requirements))
...
def tests(session: Session) -> None:
    """Run the test suite."""
    session.install(".")
    session.install("coverage[toml]", "pytest", "pygments", "pytest-benchmark")
    args = session.posargs or ["--benchmark-disable"]
    try:
